...
- **The reader keeps and hands back the file you imported.** Ripple parsed an upload into scenes and units and dropped the file itself, so there was no way to download the original or re-upload it later. The import now keeps the bytes, and an **Original** button in the reader downloads them byte for byte; deleting the script removes the kept file. On a Replit deployment the copy is mirrored to Object Storage, which survives a redeploy where the deployment filesystem does not; the SDK is an optional `replit` extra, imported only when `REPL_ID` is set, so the base install and the tests never pull it.
...
- **Ripple runs on Replit, and the Postgres driver is pinned for either URL scheme.** A live instance is on Replit Autoscale at mohammedshehu.com/ripple, with PostgreSQL for storage and Object Storage for imported originals; the same code runs on SQLite locally, which is what the tests use. `DATABASE_URL` with the `postgres://` or the `postgresql://` scheme now resolves to psycopg 3, since either otherwise defaults to psycopg2, which the deployment does not carry, and the `[replit]` extra declares `psycopg[binary]` beside the Object Storage client. The pool already pre-pings and recycles, so a request after the database's idle timeout reconnects instead of failing.
...
- **The reader toolbar's buttons are one height, and the pane toggle no longer clips.** The buttons carrying a leading glyph (View graph, Ask Ripple, See ripple) rendered taller than the plain ones because nothing fixed their line height, so a decorative character stretched the box; on Safari the `<button>` elements took their own metrics on top of that, and the pane-toggle glyph was cut by its own tight line box. Every button now lays out through a flex box with a fixed line height and a cleared native appearance, so an `<a>`, `<span>`, and `<button>` all match, and the toggle has room for its glyph. Most visible on a tablet, where the toolbar wraps onto two rows.

- **The first page load after an idle period no longer fails on Replit.** The deployment's Postgres closes a connection left idle for a few minutes, and the connection pool had no liveness check, so the next request drew that dropped connection and returned the server-error page; only a reload succeeded, once the pool had replaced the connection. The engine now tests a connection before handing it out and retires one before the server's own idle cutoff, so the deployment recovers on its own without a reload. SQLite, used locally and in the tests, keeps its default pool: it has no server to drop a connection, so the check would cost without protecting anything.

- **The reader holds together in a narrow window.** Resizing the window small left the sidebar drawer open over the page, so the script vanished behind it, and the toolbar's last buttons were clipped off the edge. The sidebar now shuts as the window crosses to narrow and the content stays put; the toolbar wraps its buttons onto more rows instead of cutting them off; and Requirements, Local graph, and Continuity each fold to their header, so where the pane stacks under the script the reader can reclaim the height. The left and right panes stay collapsible by their own toggles.
