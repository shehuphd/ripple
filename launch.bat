...
rem Find a free port, starting at the default and trying up to twenty above
rem it. The kill above already cleared any same-app holder, so a busy port
rem here belongs to something else and is skipped rather than taken.
set "PORT="
...
rem the first page load is not a connection error.
...
rem response is sent no-store.
"%PYTHON%" -m uvicorn ripple.web.app:app --host 127.0.0.1 --port %PORT% --log-level info --reload --reload-dir "%CD%\ripple"
