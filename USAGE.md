...
**Conversations and questions.** The sidebar lists the script's conversations above its single-question history, both replayed from storage at no cost. A conversation is named by its opening request; closing one rejects any proposal it left pending. Inside an open conversation every message goes to the agent, which is holding the context.
