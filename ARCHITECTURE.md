...
│   ├── llm/            # Provider contract and the KeyCall-backed adapter
...
| `ripple/llm/` | `base.py` is the provider contract (`ModelInfo`, `GenerationResult`, `ProviderError`) plus provider-agnostic helpers (`infer_tier`, `is_text_model`); `keycall_provider.py` is the one adapter, backed by [KeyCall](https://pypi.org/project/keycall), that every registered provider uses; `fixture.py` is a deterministic test double that never touches a network. |
