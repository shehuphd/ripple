...
- **`keycall`:** the LLM adapter behind extraction, the preview's judgement, extraction, and continuity passes, and the prose explanation, for its native schema enforcement and typed errors
- **`google-genai`:** Google's own SDK, behind the Ask Ripple path (the grounded query, the agent's turns, and its drafter), so a Google Cloud SDK generation is imported and called at runtime; same credential and model ids as the KeyCall path, mapped to the same `GenerationResult`
- **`traceact`:** startup-configured full-payload tracing (`capture_inputs=True`, `capture_outputs=True`), so every prompt, reply, screenplay excerpt, and filename is written to the local trace log; the sole guard is the value-pattern redaction (`redact_values=True`) that catches credential-shaped strings
...
| `ripple/graph/` | `diff.py` compares two assertion sets by edge identity (both endpoints plus predicate), not row identity, so a proposed-but-unsaved assertion still compares correctly; `continuity.py` retrieves the bounded evidence packet the continuity judgement consumes and computes the two findings that need no model (an edit removing an entity's only `establishes` edge while later scenes still reference it, and a character renamed on one line while the old name survives on others); `layout.py` is a fixed, deterministic 2D layout (focus at centre, scenes on a spine, entities in per-department wedges) rather than a force simulation, so two renders of the same graph match; `predicates.py` is the shared vocabulary both the schema and the diff read from; `fixtures.py` builds judge-visible scene context for the preview prompt. |
...
| `ripple/tracing.py` | Configures TraceAct once at startup: full-payload capture (`capture_inputs=True`, `capture_outputs=True`, `redact_by_default=False`, no redaction presets), so a run is diagnosed from its complete prompts and replies; `redact_values=True` still catches credential-shaped strings, and the trace log stays local. |
...
|---|---|---|
| On write, `extraction/service.py` | Each proposed entity against the graph so far | Normalized canonical name within script and type; then a recorded alias, but only when one entity holds it; then, for cast only, a strict whole-word name extension, and only when one character stands in that relation. Aliases widen an entity rather than replace what an earlier scene learned. |
| After the fact, `services/duplicates.py` | Forks already in the graph | Same-type pairs on two signals, one canonical name contained whole in the other, or one name recorded as the other's surface form. Pronouns identify nobody and are ignored. The survivor is the entity more of the graph cites; a person decides, and "keep separate" is recorded so the pair is never offered again. |
| Across drafts, `services/renames.py` | One character under two names | Positional evidence: dialogue that carried over word-for-word but changed speaker. Three or more transferred positions covering 80% applies the rename; 50% to 80% becomes a finding to confirm or dismiss; any line still speaking under the old name is reported as a partial rename. |

A merge or an applied rename moves every assertion, attribute, alias, and speaker link onto the survivor through one absorption path, keeps the absorbed name as an alias so later mentions resolve, records the decision as an accepted change set, and bumps the script version so a preview drafted against the pre-merge graph refuses as stale.
...
- **Full trace payloads stay on the local machine.** `TraceConfig(capture_inputs=True, capture_outputs=True, redact_by_default=False)` records every prompt, reply, screenplay excerpt, and filename, so a failed run is diagnosed by reading its trace. The guard is where the traces live: `data/traces/` is gitignored and never leaves the machine, and `redact_values=True` still catches credential-shaped strings. Nothing here is sent anywhere.
