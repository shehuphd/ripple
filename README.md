...
**See what the name means.** With the app open, press `Ctrl` `Opt` `Space`. The tab becomes a dark lake: stones drop, ripples spread, and as each wavefront passes a point a production entity surfaces there, faint lines pairing the ones that come up together. That is the idea the tool is built on, drawn rather than explained. Escape brings the app back where you left it, down to the scroll position.
